...
public class NewDriving : MonoBehaviour {
    ...
    private float nitroTorque;
            
    private float originalSpeed;
    private float originalMaxSpeed;
    private int originalNitroForce;
    private float originalTorque;

    // android turn controls
    ...
    void Awake () {
        ...
        originalSpeed = maxSpeed-200;
        originalMaxSpeed =  maxSpeed;
        originalNitroForce = turboForce;
        originalTorque = maxTorque;
        nitroSpeed = maxSpeed + 200;
        nitroMaxSpeed = maxSpeed + 200;
        nitroTorque = maxTorque + 200;
        nitroForce = turboForce + 1000000;
    ...
    void FixedUpdate () {
            ...
            if(isNitroOn == true)
            {
                ...
                turboForce = nitroForce;
                isNitroOn = false;
                StartCoroutine("NitroOff", 3);
            }
            ...
            if (currentSpeed != 0)
            {
                flWheelCollider.steerAngle = speedControlledMaxSteerAngle*turnValue;
                    //Input.GetAxis("Horizontal");
                frWheelCollider.steerAngle = speedControlledMaxSteerAngle*turnValue;
                    //Input.GetAxis("Horizontal");
            }
    ...
    IEnumerator NitroOff(int seconds)
    {
        yield return new WaitForSeconds(seconds);
        maxSpeed = originalMaxSpeed;
        maxTorque = originalTorque;
        turboForce = originalNitroForce;
        GetComponent<CarScript2>().nitroSpark.renderer.enabled = false;
    }
    ...
    void OnTurnLeftRelease()
    {
        turnValue = 0;
    }

    void OnTurnRightRelease()
    {
        turnValue = 0;
    }
